...
app.get('/api/pay/status', (req, res) => {
  ...
  if (!token || token.length < 10) return res.json({ code: 0, data: { paid: false } });
...
// 首次获取 adminToken（用于查看订单）
app.get('/api/pay/admin-token', (req, res) => {
  const data = loadJSON(PAY_FILE);
  if (!data.adminToken) { data.adminToken = crypto.randomBytes(16).toString('hex'); saveJSON(PAY_FILE, data); }
  res.json({ code: 0, data: { adminToken: data.adminToken } });
});
